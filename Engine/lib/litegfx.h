...
void ltex_drawrotsized(const ltex_t* tex, float x, float y, float angle, float pivotx, float pivoty, float width,
                       float height, float u0, float v0, float u1, float v1);
void ltex_bindcolor(const ltex_t* tex);

/* vertex */

using lrendermode_t = enum
{
    MODE_POINTS,
    MODE_LINES,
    MODE_TRIANGLES,
...
lvert_t lvert(float x, float y, float z, float nx, float ny, float nz, float u, float v, float r, float g, float b,
              float a);
